...
provide the sequencing references. Sixteen complete originals and14 C
payloads are archived under0.79.13/hdmi-link-20260912. Full source notices
are retained in gsp_hdmi_link.zig. The identical354786-byte module/image
runtime license adds the original nvtiming.h and hdmi_spec.h MIT notices;
its former352032-byte prefix is preserved. R4GFX EDID flag facts follow
the already pinned MIT libdisplay-info source and its existing license.
Original R4OS state/ownership policy is Apache-2.0. No firmware changed;
the original-header C generator is host-only.
...
The unchanged R4GFX parser/timings and original library LICENSE/NOTICE and
third-party notices are pinned alongside them. Original R4OS capture policy
and synthetic RFO fixtures: Apache-2.0. NVIDIA44 product remains byte-identical.
...
original notices in gsp_event_objects.zig. Original R4OS bounded lifecycle,
receipt and owner integration: Apache-2.0. NVIDIA 0.1.44 remains byte-identical.


GSP runtime notifications (0.79.10, 2026-09-11):
Fixed NVIDIA 570.144 wire/notification layouts and Nouveau framing: MIT.
Eleven full pinned originals under runtime-events-20260911. New source retains
four full source MIT blocks and the package-default MIT notice (COPYING) for
rpc_global_enums.h, which has no own header. Original R4OS bounded owner
delivery/receipt policy: Apache-2.0. NVIDIA 0.1.44 product remains byte-identical.


GSP runtime sequencer (0.79.10, 2026-09-11):
Original R4OS receipt/ownership/deadline integration, Apache-2.0. Existing
sequencer and GA106 core semantics reused, no new register or wire layout.
Twelve full hash-pinned originals under runtime-sequencer-20260911; existing
source MIT notices retained. NVIDIA0.1.44 R4D/resources remain byte-identical.
...
gsp_transport retains its original notice plus the complete kernel_gsp.c
notice for notify-after-publication. Eight full pinned sources accompany
queue-notify-20260911. Original R4OS ownership policy is Apache-2.0.
Source is host-linked only; product R4D and packaged notices are unchanged.
