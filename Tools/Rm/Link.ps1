...
if($adapter.schema -ne 5 -or $adapter.subset -cne 'cpu-memory-clock-semaphores-waits-and-logging' -or $adapter.components.Count -ne 12 -or !$adapter.host_acceptance.passed -or !$adapter.clock_acceptance.passed -or !$adapter.semaphore_acceptance.passed -or !$adapter.wait_acceptance.passed -or !$adapter.format_acceptance.passed -or $adapter.driver_wait_provider_linked -or $adapter.driver_log_provider_linked -or $adapter.runtime_complete -or $adapter.driver_heap_provider_linked -or $adapter.driver_clock_provider_linked -or $adapter.driver_semaphore_provider_linked -or $adapter.native_fault_provider_linked -or $adapter.gpu_executed -or $adapter.module_installed){throw 'Verified CPU memory, clock, semaphore, wait and logging adapter subsets are required'}
...
foreach($component in $plan.components) {
    ...
    if($componentAdapters.Count -ne 6){throw 'Memory, clock, semaphore, wait, formatting and logging adapters are required for each component'}
