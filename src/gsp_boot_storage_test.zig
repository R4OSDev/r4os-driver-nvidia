...
var closing = false;
var clock: u64 = 100;
var current_pack: storage.Pack = .{ .bytes = storage.pack_bytes, .signature = storage.signature_offset, .metadata = storage.metadata_offset };
var device_pack: [45056]u8 = @splat(0xa5);
...
fn allocate(bytes: u64, alignment: u32, out: *a.DriverHeapAllocation) callconv(.c) i32 {
    ...
    std.debug.assert(bytes == (if (index == 0) @as(u64, 63676416) else current_pack.bytes));
...
fn map(info: *const a.DmaPinnedBuffer, constraints: *const a.DmaConstraints, direction: u32, out: *a.DmaMapping) callconv(.c) i32 {
    ...
    if (index == 4) {
        std.debug.assert(std.mem.allEqual(u8, data[0..current_pack.signature], 0x42));
        std.debug.assert(std.mem.allEqual(u8, data[current_pack.signature..current_pack.metadata], 0x53));
        std.debug.assert(std.mem.allEqual(u8, data[current_pack.metadata..current_pack.bytes], 0));
        @memcpy(device_pack[0..current_pack.bytes], data[0..current_pack.bytes]);
    ...
    if (index == 4) switch (fault) {
        ...
        .timeout => clock = 1100,
        .overlap => out.segments[0].phys_addr = 0x100000000 - current_pack.metadata, // Only metadata overlaps GSP.
...
fn sync(mapping: *const a.DmaMapping) callconv(.c) i32 {
    ...
    sync_count += 1;
    if (index == 4) {
        const meta = backing[1].?[current_pack.metadata..];
        std.debug.assert(word(meta, 0) == wpr.magic and word(meta, 16) == 0x100000000);
        std.debug.assert(word(meta, 32) == 0x200000000 and word(meta, 72) == 0x200000000 + current_pack.signature);
        std.debug.assert(std.mem.allEqual(u8, meta[200..], 0));
        std.debug.assert(word(device_pack[current_pack.metadata..], 0) == 0);
        if (fault == .sync) return -1;
        @memcpy(device_pack[0..current_pack.bytes], backing[1].?);
    }
    return 0;
}
...
test "firmware CPU storage boot pack owns complete GSP lifetime and resynchronizes metadata before publication" {
    ...
    @memset(image, 0x79);
    const boot_image: [36864]u8 = @splat(0x42);
    const signature: [4096]u8 = @splat(0x53);
    for ([_]u16{0x176,0x192}) |chip| {
    current_pack = try storage.packFor(chip);
    const spec = @import("firmware.zig").bootSpecification(chip).?;
    // The existing GA fixture and the actual AD102 descriptor are separately
    // hash-admitted by WPR preparation. Boot body remains a DMA sentinel.
    ...
    } else @memcpy(&descriptor, @import("booter_storage_test.zig").asset(&spec.descriptor));
    const sources = storage.Sources{
        ...
        .image = image,
        .boot_image = boot_image[0..current_pack.signature],
        .descriptor = &descriptor,
        .signature = &signature,
    };
    for (std.enums.values(Fault)) |case| {
        if (chip != 0x176 and case != .none and case != .bounce and case != .overlap) continue;
        fault = case;
        clock = 100;
        closing = false;
        allocated = 0;
        ...
        if (failure) |expected| {
            ...
            try t.expectEqual(@as(u64, 0x200000000 + current_pack.metadata), report.metadata_address);
            try t.expectEqual(@as(usize, 5), sync_count);
            try t.expectEqual(case == .bounce, report.pack_bounced);
            try t.expectEqualSlices(u8, backing[1].?, device_pack[0..current_pack.bytes]);
            try t.expect(wpr.matchesPlan(device_pack[current_pack.metadata..][0..wpr.bytes], &owned.vram_plan.?));
            if (case == .none) {
