//! Resident GSP image plus a contiguous, generation-sized boot/signature/metadata pack.
...
pub const Pack = struct { bytes: usize, signature: usize, metadata: usize };
pub fn packFor(chip: u16) !Pack {
    const spec = @import("firmware.zig").bootSpecification(chip) orelse return error.UnsupportedChip;
    if (spec.image.bytes == 0 or spec.image.bytes % radix.page_bytes != 0) return error.WrongBootSize;
    return .{ .bytes = spec.image.bytes + wpr.signature_bytes + radix.page_bytes,
        .signature = spec.image.bytes, .metadata = spec.image.bytes + wpr.signature_bytes };
}
...
pub const Report = struct {
    ...
    boot_bytes: usize = boot.image.bytes,
    pack_bounced: bool,
    app_version: u32 = 0,
};
pub const Storage = struct {
    ...
    pub fn stageAdmitted(self: *Storage, ctx: *const r4os.r4dev.DriverContext, sources: *const Sources, timeout_ns: u64) Error!Report {
        ...
        const pack = try packFor(sources.chip_id);
        if (sources.boot_image.len != pack.signature) return error.WrongBootSize;
        ...
        if (self.heap.?.allocate(pack.bytes, radix.page_bytes, &self.allocation) != a.driver_heap_ok) return error.Memory;
        const allocation = &self.allocation;
        if (allocation.handle == 0 or allocation.cpu_address == 0 or allocation.cpu_address & 4095 != 0 or
            allocation.byte_length != pack.bytes or allocation.alignment < radix.page_bytes or
            allocation.cpu_address > std.math.maxInt(u64) - pack.bytes) return error.Memory;
        _ = try self.checkClock();
        const data: [*]u8 = @ptrFromInt(allocation.cpu_address);
        const output = data[0..pack.bytes];
        @memset(output, 0);
        @memcpy(output[0..pack.signature], sources.boot_image);
        @memcpy(output[pack.signature..pack.metadata], sources.signature);
        ...
        if (self.pin.version != 1 or self.pin.size < @sizeOf(a.DmaPinnedBuffer) or self.pin.handle == 0 or
            self.pin.virt_addr != allocation.cpu_address or self.pin.bytes != pack.bytes or
            self.pin.page_count != pack.bytes / radix.page_bytes or self.pin.flags != 0 or self.pin.reserved != 0) return error.Descriptor;
        _ = try self.checkClock();
        const constraints = a.DmaConstraints{
            .dma_mask = radix.dma_mask,
            .alignment = radix.page_bytes,
            .max_segment_bytes = @intCast(pack.bytes),
            .max_segments = 1,
        ...
        if (map.version != 1 or map.size < @sizeOf(a.DmaMapping) or map.handle == 0 or map.pin_handle != self.pin.handle or
            map.requested_bytes != pack.bytes or map.mapped_bytes != pack.bytes or map.direction != a.dma_direction_to_device or
            map.segment_count != 1 or map.reserved0 != 0 or map.reserved1 != 0 or
            (map.flags & ~a.dma_mapping_flag_bounced) != constraints.flags or
            map.segments[0].reserved != 0 or map.segments[0].bytes != pack.bytes) return error.Descriptor;
        const address = map.segments[0].phys_addr;
        if (address == 0 or address > radix.dma_mask or pack.bytes - 1 > radix.dma_mask - address) return error.Address;
        ...
        for (self.image.segments[0..self.image.segment_count]) |segment| {
            if (address < segment.address + segment.bytes and segment.address < address + pack.bytes) return error.Overlap;
        }
        const metadata = try wpr.encode(&input, &.{
            .gsp_segments = self.image.segments[0..self.image.segment_count],
            .boot_image = .{ .address = address, .bytes = pack.signature },
            .signature = .{ .address = address + pack.signature, .bytes = wpr.signature_bytes },
        });
        @memcpy(output[pack.metadata..][0..metadata.len], &metadata);
        ...
        self.report = .{
            ...
            .signature_address = address + pack.signature,
            .metadata_address = address + pack.metadata,
            .pack_bytes = pack.bytes, .boot_bytes = pack.signature,
