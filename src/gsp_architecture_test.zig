...
pub fn check() !void {
    ...
    for ([_]u16{0x172,0x173,0x174,0x176,0x177,0x192,0x193,0x194,0x196,0x197}) |chip| {
        ...
        try t.expect(result.data_bytes == 120 and result.revision == 1 and std.mem.allEqual(u8, result.data[120..], 0));
        try t.expect(info.memory_generation == va.epoch and info.va_start == va.base and info.va_end == va.base + va.bytes);
        try t.expect(info.gpc_count == 2 and info.tpc_count == 7 and info.vram_bytes == 12 << 30 and info.flags == 0);
