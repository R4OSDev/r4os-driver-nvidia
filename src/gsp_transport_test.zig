...
const Model = struct {
    ...
    fail_publish: ?ring.Location = null,
    ...
    fn publish(context: *anyopaque, limit: u64, queue: ring.Queue, offset: usize, input: []const u8) !void {
        ...
        if (self.fail_publish) |location| if (location.queue == queue and location.offset == offset) return error.PortFailure;
        try self.finish();
    }
...
test "GPU virtual ranges keep independent addresses and exact memory ownership" {
    ...
    for ([_][]const u8{ "control", "part" }) |kind| {
        ...
        const binding: legacy.Binding = .{ .space = .{ .epoch = 7, .client = 0xc1d00000, .device = 0x10000000,
            .handle = 0x10000006, .base = 0x200000, .bytes = 0x100000000, .big_page_bytes = 65536 }, .memory = 0x10000007, .virtual = 0x10000008 };
        const is_part = std.mem.eql(u8, kind, "part");
        const golden: []const u8 = if (is_part) @embedFile("fixtures/buffer-part-570.144.bin") else @embedFile("fixtures/control-buffer-570.144.bin");
        const part: legacy.Part = if (is_part) .{ .total_bytes = 80 * 1024 * 1024, .offset = 8175 * 4096, .byte_length = 12288 } else .{};
        var offset: usize = 0;
        var address: u64 = 0;
        var request: [160]u8 = undefined;
        for (std.enums.values(legacy.Operation)) |op| {
            const encoded = try legacy.encodePart(binding, part, op, &.{ 0x100000000, 0x300000000, 0x100004000 }, address, &request);
            const size = encoded.bytes.len;
            try t.expectEqualSlices(u8, golden[offset..][0..size], encoded.bytes);
            if (op == .allocate) address = 0x600000;
            offset += size * 2;
        }
        try t.expect(offset == golden.len);
    }
    {
        // Generated by the original C allocation/RPC types, independently of
        // the Zig encoder. In particular public NVOS47 is NOT this wire ABI.
        const golden = @embedFile("fixtures/virtual-range-570.144.bin");
        const plan: wire.Allocation = .{ .space = .{ .epoch = 7, .client = 0xc1d00000, .device = 0x10000000,
            .handle = 0x10000006, .base = 0x200000, .bytes = 0x100000000, .big_page_bytes = 65536 },
            .object = 0x10000008, .bytes = 128 * 1024, .alignment = 65536, .fixed_address = 0x800000 };
        var map: wire.Mapping = .{ .allocation = plan, .address = 0x800000, .memory = 0x10000007, .memory_bytes = 65536,
            .memory_offset = 8192, .virtual_offset = 32768, .bytes = 16384, .location = .system };
        var request: [160]u8 = undefined;
        var offset: usize = 0;
        for (0..4) |index| {
            const encoded = switch (index) {
                0 => try wire.allocate(plan, &request),
                1, 2 => try wire.mapping(map, if (index == 1) .map else .unmap, &request),
                else => try wire.free(plan, &request),
            };
            const size = encoded.bytes.len;
            try t.expectEqualSlices(u8, golden[offset..][0..size], encoded.bytes);
            const record: message.Record = .{ .shape = .{ .message_bytes = size + 80, .checksum_bytes = size + 80, .storage_bytes = 4096, .elements = 1 },
                .queue_sequence = 0, .rpc = .{ .function = encoded.function, .result = 0 }, .payload = golden[offset + size ..][0..size] };
            const reply = switch (index) { 0 => try wire.allocated(plan, record), 1, 2 => try wire.mapped(map, if (index == 1) .map else .unmap, record), else => try wire.freed(plan, record) };
            try t.expect(reply == .ok and reply.ok == @as(u64, switch (index) { 0 => 0x800000, 1, 2 => 0x808000, else => 0 }));
            offset += 2 * size;
        }
        try t.expect(offset == golden.len);
        map.memory_offset = 65536;
        try t.expectError(error.Bounds, wire.mapping(map, .map, &request));
        map.memory_offset = 8192;
        map.virtual_offset = plan.bytes;
        try t.expectError(error.Bounds, wire.mapping(map, .map, &request));
    }
    const Scenario = enum { success, rejected, wrong_address, ack, timeout, free_rejected, unmap_rejected, allocation_rejected };
    for (std.enums.values(Scenario)) |scenario| {
        ...
        try t.expect(owner.info().?.address == 0x800000);
        var copied = owner;
        try t.expect(copied.info() == null);
        var source_set: range.alias.Set = .{};
        var binding: range.Binding = .{};
        const source: range.alias.Source = .{ .space = space, .object = try memory_names.object(0),
            .allocation_bytes = 65536, .offset = 8192, .bytes = 16384, .location = .system };
        try source_set.acquire(&binding.source, source);
        ...
        std.mem.writeInt(u64, reply[40..48], owner.address + 32768 + @as(u64, if (scenario == .wrong_address) 4096 else 0), .little);
        if (scenario == .rejected) put(&reply, 48, 0x57);
        if (scenario == .timeout) model.now = deadline else {
            try model.replyRpc(&session, .{ .function = 14, .result = 0 }, reply[0..56]);
            if (scenario == .ack) model.fail_publish = session.link.?.status_read;
        }
        if (scenario == .wrong_address or scenario == .ack or scenario == .timeout) {
            try t.expectError(switch (scenario) { .wrong_address => error.Bounds, .ack => error.Io, else => error.Deadline }, owner.poll());
            try t.expect(owner.state == .failed and !source_set.empty() and !binding.mapped and owner.bindings == &binding);
            try virtualResetCheck(model, &owner, &source_set);
            continue;
        }
        try t.expect((try owner.poll()) == null and owner.state == .ready);
        token = try owner.handoff(deadline);
        if (scenario == .rejected) {
            try t.expect(source_set.empty() and binding.owner == null and binding.rejected.? == 0x57 and owner.rejected.? == 0x57);
        } else {
            try t.expect(binding.mapped and !source_set.empty());
            try t.expectError(error.Busy, binding.source.close(true));
            try t.expectError(error.Busy, owner.beginDestroy(&token, deadline, true));
            try t.expectError(error.Busy, owner.beginUnmap(&token, &binding, deadline, false));
            var second: range.Binding = .{};
            try source_set.acquire(&second.source, source);
            try t.expectError(error.Busy, owner.beginMap(&token, &second, 32768, deadline));
            try t.expect(!token.claimed and second.owner == null);
            try owner.beginMap(&token, &second, 65536, deadline);
            try virtualReply(model, &owner, 0);
            token = try owner.handoff(deadline);
            try t.expect(binding.mapped and second.mapped);
            try owner.beginUnmap(&token, &binding, deadline, true);
            if (scenario == .unmap_rejected) {
                try t.expectError(error.FirmwareResult, virtualReply(model, &owner, 0x57));
                try t.expect(binding.mapped and second.mapped and !source_set.empty());
                try virtualResetCheck(model, &owner, &source_set);
                continue;
            }
            try virtualReply(model, &owner, 0);
            token = try owner.handoff(deadline);
            try t.expect(!binding.mapped and second.mapped and !source_set.empty());
            try owner.beginUnmap(&token, &second, deadline, true);
            try virtualReply(model, &owner, 0);
            token = try owner.handoff(deadline);
        }
        try t.expect(source_set.empty() and owner.bindings == null);
        try owner.beginDestroy(&token, deadline, true);
        if (scenario == .free_rejected) {
            try t.expectError(error.FirmwareResult, virtualReply(model, &owner, 0x57));
            try t.expect(owner.namespace_live and owner.address != 0);
            try virtualResetCheck(model, &owner, &source_set);
        } else {
            try virtualReply(model, &owner, 0);
            token = try owner.handoff(deadline);
            try t.expect(owner.state == .finished and !owner.namespace_live);
            try session.rm_names.retireChildren(memory_names);
            try session.rm_names.retire(parent);
        }
        // Explicit fixed-address allocation must neither slide nor overflow.
        var plan = owner.plan;
        plan.fixed_address = 0x801000;
        @memset(&reply, 0xa5);
        try t.expectError(error.Bounds, wire.allocate(plan, &reply));
        for (reply) |v| try t.expect(v == 0xa5);
        plan = owner.plan;
...
fn virtualReply(model: *Model, owner: *@import("gsp_virtual_range.zig").Owner, status: u32) !void {
    try t.expect((try owner.poll()) == null and owner.exchange.phase == .waiting);
    owner.exchange.phase = .prepared;
    try t.expect(owner.matches(&owner.exchange, deadline));
    var moved = owner.*;
    try t.expect(!moved.matches(&owner.exchange, deadline));
    owner.exchange.phase = .waiting;
    const state = owner.state;
    var response: [160]u8 = undefined;
    const size = owner.request_bytes;
    @memcpy(response[0..size], owner.request[0..size]);
    const status_at: usize = switch (state) { .creating => 16, .mapping => 48, .unmapping => 32, .destroying => 12, else => unreachable };
    put(&response, status_at, status);
    if (state == .creating) {
        std.mem.writeInt(u64, response[112..120], 0x800000, .little);
        std.mem.writeInt(u64, response[120..128], owner.plan.bytes - 1, .little);
    } else if (state == .mapping) {
        std.mem.writeInt(u64, response[40..48], owner.address + owner.active.?.mapping.?.virtual_offset, .little);
    }
    model.peerPut(owner.exchange.session.link.?.command_read, owner.exchange.session.tx_write);
    try model.replyRpc(owner.exchange.session, .{ .function = owner.exchange.function, .result = 0 }, response[0..size]);
    _ = try owner.poll();
}
fn virtualResetCheck(model: *Model, owner: *@import("gsp_virtual_range.zig").Owner, source_set: *@import("gsp_memory_alias.zig").Set) !void {
    const reset = @import("gsp_reset.zig");
    var modeled: reset.Reset = .{ .epoch = model.epoch, .triggered = true, .io = .{
        .context = model, .generation = Model.generation, .now_ns = Model.clock,
        .admit = ResetNoIo.admit, .pci_read = ResetNoIo.pciRead, .pci_write = ResetNoIo.pciWrite,
        .read32 = ResetNoIo.read, .write32 = ResetNoIo.write,
    } };
    modeled.self_address = @intFromPtr(&modeled);
    const proof: reset.Quiescence = .{ .owner = &modeled, .epoch = model.epoch };
    try t.expectError(error.Retained, owner.closeAfterReset(proof));
    try t.expect(owner.state == .failed and owner.namespace_live);
    // A modeled reset completion is sufficient for the host ownership test;
    // this is not evidence of a physical reset or a successful GPU mapping.
    modeled.phase = .complete;
    var slices: usize = 0;
    while (!try owner.closeAfterReset(proof)) {
        slices += 1;
        try t.expect(slices <= 2);
    }
    try t.expect(source_set.empty() and owner.bindings == null and owner.state == .finished and !owner.namespace_live);
}
const ResetNoIo = struct {
    fn admit(_: *anyopaque, _: u64) !void { return error.UnexpectedResetIo; }
    fn pciRead(_: *anyopaque, _: u16) !u32 { return error.UnexpectedResetIo; }
    fn pciWrite(_: *anyopaque, _: u16, _: u32) !void { return error.UnexpectedResetIo; }
    fn read(_: *anyopaque, _: u32) !u32 { return error.UnexpectedResetIo; }
    fn write(_: *anyopaque, _: u32, _: u32) !void { return error.UnexpectedResetIo; }
};
