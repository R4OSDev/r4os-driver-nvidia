    ...
    if (target.phase == .ready) {
        ...
        if (scenario == .rm_vaspace_reject) {
            ...
            if (running.nativeControlBuffer()) |info| {
                try t.expect(info.address == 0x600000 and info.bytes == @import("gsp_control_storage.zig").bytes and info.epoch == session.epoch and buffer_requests == 3);
...
const NativeCommon = struct {
    ...
    fn modeStatus(ticket: u64, out: *a.GfxModeStatus) callconv(.c) i32 {
        std.debug.assert(automatic != null and automatic.?.ticket == ticket);
        if (mode_receipt) |receipt| {
            std.debug.assert(receipt.outcome == a.gfx_output_outcome_applied);
            if (receipt.operation == a.gfx_mode_operation_apply) {
                mode_job.?.sequence += 1; mode_job.?.operation = a.gfx_mode_operation_confirm;
    ...
    fn takeMode(input: *const a.GfxBackendBinding, out: *a.GfxDriverModeJob) callconv(.c) i32 {
        std.debug.assert(modes_enabled and std.meta.eql(input.*, @import("gsp_copy_test_model.zig").Model.binding));
        if (mode_job == null or mode_taken) return 0;
        mode_taken = true; out.* = mode_job.?; return a.gfx_output_ok;
    }
...
fn checkNativeProduct(target: *@import("gsp_device.zig").Device, table: *a.DriverApi, scenario: []const u8) !void {
    ...
    if (success) {
        ...
        if (NativeCommon.is("context_native_jobs")) {
            checkpoint = "common native mode jobs";
            try checkNativeModeJobs(target);
            checkpoint = "hotplug with resized receiver";
            try checkNativeHotplug(target, true);
        }
        if (NativeCommon.is("context_native_unknown")) {
...
fn checkNativeDetach(target: *@import("gsp_device.zig").Device) !void {
    ...
    for (0..@as(usize, if (extended_case) 1 else 2)) |iteration| {
        ...
        try run.validateDisplayDetach();
        run.display_work.?.window.?.config.detach_sor = (mode.signal.sor + 1) % 8;
        try t.expectError(error.Stale, run.validateDisplayDetach());
        run.display_work.?.window.?.config.detach_sor = mode.signal.sor;
        for (0..30) |_| {
            clock += 1000; _ = target.step();
            ...
            if (work.core.phase == .rewind or work.core.phase == .submitted) {
                ...
                if (work.core.phase == .submitted) {
                    const word_ptr: *u32 = @ptrFromInt(work.core.notifier.cpu.cpu_address);
                    word_ptr.* = 2 << 30;
                }
            }
            const window = &work.window.?;
            if (window.phase == .rewind or window.phase == .submitted) {
        ...
        try t.expect(table_owner.table.count == count and vm.released == released and
            @import("gsp_copy_test_model.zig").Model.heldReferences() == refs and
            (run.cursor_storage == null or run.cursor_storage.?.active == null));
        const copy_model = @import("gsp_copy_test_model.zig").Model;
        const completed = copy_model.completed;
        const fifo = run.fifos[product.copy.?.slot].owner.?;
        const issued = fifo.ring.issued;
        try copy_model.enqueuePresent(0, 0, 1, 1);
        clock += 1000; _ = target.step();
        try t.expect(target.phase == .ready and copy_model.completed == completed + 1 and
            copy_model.result == a.gfx_queue_result_cancelled and run.copy_job == null and
            fifo.ring.issued == issued and copy_model.heldReferences() == refs);
        clock += 1000; _ = target.step();
        // The same resources can be scanned again only through a fresh mode
        // query and the normal interlocked commit, never a forged stop receipt.
        const detached_offset = note.offset;
        try beginReceiverImage(target, previous.image.dma, mode.receiver_mode_id, clock + std.time.ns_per_s);
...
fn checkNativeHotplug(target: *@import("gsp_device.zig").Device, resize: bool) !void {
    ...
    if (resize) {
        ...
        @memset(capture.bytes[72..90], 0); // This monitor offers only the first real EDID timing.
        var sum: u8 = 0; for (capture.bytes[0..127]) |byte| sum +%= byte;
    ...
    const table_owner = run.display_resources_slot.owner.?;
    const count = table_owner.table.count; const released = vm.released;
    const mode = run.display_images[product.mode.?.window].?.boot_mode.?;
    const note = table_owner.publishedNotifier(product.window.?.slot).?;
    const words: [*]u32 = @ptrFromInt(note.cpu.cpu_address);
    const fifo = run.fifos[product.copy.?.slot].owner.?;
    const raw: [*]const u8 = @ptrFromInt(target.port.window.cpu_address);
    const old_revision = if (CatalogModel.audio) |value| value.revision else 0;
    product.audio.attach(&target.catalog, .{ .bus_kind = 1, .bus = 0, .device = 0, .function = 1, .vendor_id = 0x10de, .device_id = 0x228b });
    product.audio.revision = old_revision;
    checkpoint = "initial audio";
    try pumpNativeAudio(target);
    try t.expect(CatalogModel.audio.?.state == a.gfx_audio_route_ready);
    checkpoint = "flip";
    ...
    for (0..100) |_| {
        ...
        if (run.display_work) |*work| {
            ...
            if (work.core.phase == .submitted) {
                const user = try push.userBase(.core, 0); dm.words[(user + 4) / 4] = dm.words[user / 4];
                const core_note: *u32 = @ptrFromInt(work.core.notifier.cpu.cpu_address); core_note.* = 2 << 30;
            }
            if (work.window.?.phase == .submitted) {
                const user = try push.userBase(.window, mode.window); dm.words[(user + 4) / 4] = dm.words[user / 4];
                words[note.offset / 4] = 2 << 30;
            }
            if (work.core.phase == .complete and work.window.?.phase == .complete) {
                if (!held_stop) {
                    try t.expect(run.display_retired[mode.window] == null and run.display_images[mode.window] != null);
                    held_stop = true;
                } else {
    ...
    try t.expect(held_stop and product.hotplug.phase == .receiver_wait and run.display_images[mode.window] == null and
        run.flip_cancelled == cancelled + 1 and run.flip_visible == visible and run.flip_released == finished and
        copy.completed == completed + 1 and copy.result == a.gfx_queue_result_cancelled and !NativeCommon.published);
    checkpoint = "receiver capture";
    // Feed a complete independent acquisition fixture to the product owner.
    // The actual bounded RM/DDC acquisition and event debounce have their
    // existing integrated cases; this case isolates the lifecycle consumer.
    try returnReceiverFixture(run, snapshot);
    // CPU updates made with no monitor must be uploaded again on return.
    const cpu_index = run.presentation.?.surface.shadow.buffer.id - 1101;
    try t.expect(cpu_index < copy.host.len);
    @memset(&copy.host[cpu_index], 0x5a);
    var fetched = false;
    var signaled = false;
    const previous_generation = product.mode.?.output_generation;
    checkpoint = "restore";
    ...
    for (0..500) |_| {
        ...
        if (interrupted and !restarted and product.hotplug.phase == .receiver_wait) {
            try t.expect(product.hotplug.source_map.lease.id == 0 and product.hotplug.source.reference.id == 0 and
                run.display_images[mode.window] == null and table_owner.table.count == count and vm.released == released);
            if (!resize) try t.expect(!run.mode_control_active and (try run.modeControlStatus(product.mode_control.?)).info == null);
            try returnReceiverFixture(run, snapshot);
            restarted = true;
        }
        if (product.hotplug.phase == .online) break;
        if (run.buffer_active != null) { try replyCopyMapping(target); continue; }
        if (run.activeChannel()) |channel| if (channel.phase == .waiting) {
        ...
        if (run.display_upload_job) |work| {
            if (work.operation.phase == .prepared and work.operation.part == 0)
    ...
    checkpoint = "online invariants";
    try t.expect(interrupted and restarted and product.hotplug.phase == .online and product.hotplug.restores == 1 and !run.display_paused and fetched and
        product.mode.?.output_generation > previous_generation and NativeCommon.published and
        run.display_images[mode.window].?.image.dma == run.presentation.?.surface.scanout.?.dma and run.display_retired[mode.window] == null);
    const memory = table_owner.publishedStorage(product.window.?.slot, run.presentation.?.surface.scanout.?.dma).?.info().?;
    checkpoint = "CPU data";
    const pixel: u8 = if (resize) 0 else 0x5a;
    const width = product.mode.?.width;
    if (!std.mem.allEqual(u8, copy.imageBytes(memory.reference.buffer.id - 801)[0..width * 4], pixel))
        std.debug.print("hotplug pixels: index={d} byte={x} CPU-buffer={d} original={d} fetched={} completed={d}\n", .{
            memory.reference.buffer.id - 801,copy.imageBytes(memory.reference.buffer.id - 801)[0],run.presentation.?.surface.shadow.buffer.id,
            copy.native_index,fetched,run.presentation.?.initial_point});
    try t.expect(std.mem.allEqual(u8, copy.imageBytes(memory.reference.buffer.id - 801)[0..width * 4], pixel));
    if (resize) try t.expect(width == 65 and product.mode.?.height == 20 and vm.released == released + run.presentation_buffers and
        table_owner.table.count == count + 1 - run.presentation_buffers and product.hotplug.source.reference.id == 0 and
        product.hotplug.source_map.lease.id == 0 and NativeCommon.automatic.?.phase == a.gfx_mode_phase_confirmed and
        !copy.replacement_lent and copy.borrowed_releases == 0);
...
fn returnReceiverFixture(run: *@import("gsp_runtime.zig").Owner, snapshot: *const @import("gsp_outputs.zig").Snapshot) !void {
    ...
    run.output_generation += 1;
    run.outputs.data = snapshot.*;
    run.outputs.data.generation = run.output_generation;
    run.outputs.data.final_receipt_serial += 1;
    run.outputs.data.coherent = true; run.outputs.invalidated = false; run.outputs.state = .returned;
    try run.receiver_events.finished(run.epoch, clock, false);
}
...
fn checkNativeModeTimeout(target: *@import("gsp_device.zig").Device) !void {
    ...
    try t.expect(product.modes.phase == .idle);
    const old = run.presentation.?;
    ...
    const mode = product.publication.modes[1];
    NativeCommon.mode_job = .{ .ticket = 1, .sequence = 1, .operation = a.gfx_mode_operation_apply,
    ...
    NativeCommon.mode_taken = false; NativeCommon.mode_receipt = null;
    var gpu_table = run.display_resources_slot.owner.?.table.image;
    var timed_out = false;
    for (0..200) |_| {
        clock += 1000; _ = target.step(); try t.expect(target.phase == .ready);
        ...
        if (run.display_upload_job) |work| if (work.operation.phase == .prepared and work.operation.part == 0)
            try pumpLiveTable(target, &gpu_table, 2);
        if (run.initial_image) |work| if (work.operation.submitted) {
            try t.expect(work.operation.gpu.lease.id != 0 and NativeCommon.mode_receipt == null and run.presentation == old);
            clock = work.deadline; _ = target.step(); timed_out = true; break;
        };
    }
...
fn checkNativeModeJobs(target: *@import("gsp_device.zig").Device) !void {
    ...
    for (0..2) |iteration| {
        const index: usize = blk: { for (native.slots, 0..) |slot, i| if (!slot.live) break :blk i; return error.Capacity; };
        const borrowed = copy.lendReplacement(index, 96, 24);
        for (&copy.host[1], 0..) |*byte, i| byte.* = @truncate(i * 31 + 9 + iteration);
        const mode = product.publication.modes[1];
        var job: a.GfxDriverModeJob = .{ .ticket = 1 + iteration * 2, .sequence = 1, .operation = a.gfx_mode_operation_apply,
            .backend = product.backend, .assignment = .{ .output = product.output, .mode_id = mode.mode_id,
                .head_id = product.mode.?.head, .plane_id = product.mode.?.window, .pll_id = product.mode.?.head,
                .source_width = mode.width, .source_height = mode.height, .destination_width = mode.width, .destination_height = mode.height,
                .buffer = borrowed.buffer }, .mode = mode, .reference = borrowed, .deadline_ns = clock + std.time.ns_per_s };
        ...
        job.operation = if (iteration == 0) a.gfx_mode_operation_rollback else a.gfx_mode_operation_confirm;
        ...
        if (iteration == 0) {
            ...
            job.ticket = 2; job.sequence = 1; job.operation = a.gfx_mode_operation_apply;
            job.mode.pixel_clock_hz += 1; job.deadline_ns = clock + std.time.ns_per_s;
            const sequence = target.session.?.tx_sequence;
            NativeCommon.mode_job = job; NativeCommon.mode_taken = false; NativeCommon.mode_receipt = null;
            try pumpNativeModeJob(target);
...
fn pumpOutputModeJob(target: *@import("gsp_device.zig").Device, product: anytype) !void {
    ...
    for (0..400) |_| {
        ...
        if (run.display_upload_job) |work| {
            if (work.operation.phase == .prepared and work.operation.part == 0)
                try pumpLiveTable(target, &gpu_table, if (run.display_resources_slot.owner.?.table.change.?.remove) 1 else 2);
        }
        if (run.initial_image) |work| if (work.operation.submitted) {
            if (!fetched) { try copy.fetch(fifo, raw[0..@intCast(target.port.window.byte_length)]); try copy.execute(); fetched = true; }
...
fn deliverOutputHead(target: *@import("gsp_device.zig").Device, head: u32, caused: bool) !void {
    ...
    words[(irqs.reg.leaf + endpoint.leaf * 4) / 4] = 0;
    words[(irqs.reg.leaf + endpoint.display.leaf * 4) / 4] = endpoint.display.bit;
    words[heads.reg.dispatch / 4] = @as(u32, 1) << @intCast(head);
    words[(heads.reg.status + head * 4) / 4] = if (caused) 2 else 0;
    words[(heads.reg.position + head * 0x800) / 4] = (@as(u32, @truncate(sequence + 1)) << 16) | 17;
    clock += 1000;
    const wakes = IrqModel.wakes;
    try t.expect(IrqModel.dispatch(IrqModel.irq) == a.irq_result_handled);
    try t.expect(!endpoint.failed() and IrqModel.wakes == wakes + 1 and
        endpoint.display.heads[head].snapshot().?.sequence == sequence + @intFromBool(caused));
    // The MMIO model does not implement W1C side effects; clear the delivered
    // cause before the next explicitly injected hardware event.
    words[(irqs.reg.leaf + endpoint.display.leaf * 4) / 4] = 0;
    words[(heads.reg.status + head * 4) / 4] = 0;
    words[heads.reg.dispatch / 4] = 0;
    words[irqs.reg.top / 4] = 0;
}
fn checkNativeFlip(target: *@import("gsp_device.zig").Device) !void {
    const run = &target.running; const product = &target.native_output;
    const copy = @import("gsp_copy_test_model.zig").Model;
    const native = @import("gsp_vram_test_model.zig").Model;
    const display = @import("gsp_display_test_model.zig").Model;
    ...
    const table_owner = run.display_resources_slot.owner.?;
    const old = run.presentation.?;
    const original = run.display_images[product.mode.?.window].?;
    const note = table_owner.publishedNotifier(product.window.?.slot).?;
    const words: [*]u32 = @ptrFromInt(note.cpu.cpu_address);
    const raw: [*]const u8 = @ptrFromInt(target.port.window.cpu_address);
    const fifo = run.fifos[product.copy.?.slot].owner.?;
    const core = &run.display_channels[product.core.?.slot].?;
    const window = &run.display_channels[product.window.?.slot].?;
    ...
    try run.uploadDisplayTable(product.engine.?, product.copy.?, deadline);
    try pumpLiveTable(target, &gpu_table, 2);
    const borrowed = copy.lendReplacement(index, original.image.width, original.image.height);
    @memset(&copy.host[1], 0x7d);
    try run.prepareDisplayPresentationImage(dma, borrowed, deadline);
    const candidate = &run.presentation_slots[1].?;
    try t.expectError(error.Stale, run.flipDisplayPresentationImage(dma, deadline));
    checkpoint = "render";
    ...
    for (0..100) |_| {
        ...
        if (run.initial_image) |work| {
            if (work.operation.submitted) {
                if (!fetched) {
                    try copy.fetch(fifo, raw[0..@intCast(target.port.window.byte_length)]); try copy.execute(); fetched = true;
                    try t.expect(candidate.initial_point == 0 and run.flip_visible == 0 and copy.initial_read.lease.id != 0);
                } else try copy.signal();
            }
        } else break;
    }
    try t.expect(run.initial_image == null and candidate.initial_point != 0 and copy.initial_read.lease.id == 0 and
        std.meta.eql(run.display_images[product.mode.?.window].?, original) and run.flip_visible == 0);
    const core_point = core.ring.issued;
    const core_put = display.words[(try push.userBase(.core, 0)) / 4];
    const before_copies = run.copy_completed;
    const before_queue = copy.completed;
    const before_rpc = target.session.?.tx_sequence;
    // Retire only an older modeled notifier use, keeping the active use BEGUN.
    words[(note.offset ^ 16) / 4] = 2 << 30;
    for (0..2) |iteration| {
        const selected = if (iteration == 0) candidate else old;
        const selected_dma = selected.surface.scanout.?.dma;
        const prior = run.display_images[product.mode.?.window].?;
        ...
        try t.expectError(error.Binding, io.admit_display_push.?(io.context, &target.port, window, deadline, .read));
        work.window.config.with_core = false;
        work.receipt.render_point += 1;
        try t.expectError(error.Binding, io.admit_display_push.?(io.context, &target.port, window, deadline, .read));
        work.receipt.render_point -= 1;
        work.previous.window_point += 1;
        try t.expectError(error.Binding, io.admit_display_push.?(io.context, &target.port, window, deadline, .read));
        work.previous.window_point -= 1;
        try t.expectError(error.Binding, io.admit_display_push.?(io.context, &target.port, core, deadline, .read));
        try t.expectError(error.Busy, run.commitDisplayCore(product.core.?, deadline));
        try t.expectError(error.Busy, run.beginCopyWork(product.copy.?, product.backend, deadline));
        clock += 1000; _ = target.step();
        try t.expect(target.phase == .ready and work.window.phase == .submitted and work.receipt.submitted_ns == clock);
        const program = window.ring.program.?;
        var at: usize = 0;
        var interlock = false;
        var present_control = false;
        var target_image = false;
        while (at < program.count) {
            const method = program.words[at] & 0x3ffc;
            const count = (program.words[at] >> 18) & 1023;
            try t.expect(count != 0 and at + 1 + count <= program.count);
            switch (method) {
                0x370 => { try t.expect(count == 2 and program.words[at + 1] == 0 and program.words[at + 2] == 0); interlock = true; },
                0x308 => { try t.expect(count == 1 and program.words[at + 1] == 1); present_control = true; },
                0x240 => { try t.expect(program.words[at + 1] == selected_dma); target_image = true; },
                else => {},
            }
            at += 1 + count;
        }
        try t.expect(interlock and present_control and target_image and core.ring.issued == core_point);
        const user = try push.userBase(.window, product.mode.?.window);
        display.words[(user + 4) / 4] = display.words[user / 4];
        clock += 1000; _ = target.step();
        ...
        const missing_irq = NativeCommon.is("context_native_flip_irq_timeout");
        const missing_notifier = NativeCommon.is("context_native_flip_notifier_timeout");
        if (iteration == 0 and !missing_irq) { try deliverNativeHead(target, true); _ = target.step(); }
        try t.expect(run.flip_visible == iteration and work.window.phase == .submitted);
        const gpu_timestamp = (@as(u64, 7) << 32) + 123 + iteration;
        if (!missing_notifier) {
            words[note.offset / 4 + 2] = @truncate(gpu_timestamp);
            words[note.offset / 4 + 3] = @truncate(gpu_timestamp >> 32);
            words[note.offset / 4] = 1 << 30;
            clock += 1000; _ = target.step();
        }
        if (iteration == 1) {
            // Window has begun but no new head IRQ has arrived. Advancing
            // CPU time, including a worker poll, does not create VBlank.
            try t.expect(work.window.phase == .complete and run.flip_visible == iteration);
            clock += 10 * std.time.ns_per_ms; _ = target.step();
            try t.expect(run.flip_visible == iteration);
        ...
        const visible = run.flip_receipts[prior.head].?;
        try t.expect(run.flip_visible == iteration + 1 and run.flip_released == iteration and visible.sequence == iteration + 1 and
            visible.previous_dma == prior.image.dma and visible.image_dma == selected_dma and visible.render_point == selected.initial_point and
            visible.begun_gpu_timestamp == gpu_timestamp and visible.begun_observed_ns >= visible.submitted_ns and
            visible.head_observation.sequence == iteration + 1 and visible.head_observation.observed_ns >= visible.submitted_ns and
            visible.previous_released_ns == 0 and !try table_owner.imageFinished(product.window.?.slot, prior.image.dma));
        try t.expect(run.display_images[product.mode.?.window].?.image.dma == selected_dma and core.ring.issued == core_point and
            display.words[(try push.userBase(.core, 0)) / 4] == core_put and run.copy_completed == before_copies and copy.completed == before_queue);
        try t.expectError(error.Busy, run.flipDisplayPresentationImage(prior.image.dma, deadline));
        try t.expectError(error.Busy, run.selectDisplayPresentationImage(selected_dma));
    ...
    try t.expect(run.presentation == old and target.session.?.tx_sequence == before_rpc);
    checkpoint = "retire inactive candidate";
    var retired = false;
    for (0..100) |_| {
    ...
    try t.expect(retired and run.presentation_slots[1] == null and copy.replacementReferences() == 0 and
        copy.borrowed_releases == 0 and copy.dma[1].lease.id == 0);
    copy.replacement_lent = false;
    ...
    try t.expect(!native.slots[index].live and run.native_active == null and native.released == before_released + 1);
}
fn checkPresentationReplacement(target: *@import("gsp_device.zig").Device) !void {
    ...
    while (iteration < 2) : (iteration += 1) {
        ...
        if (iteration == 1) {
            ...
            // slot, including the mapping reused by upload and Present.
            words[(note.offset ^ 16) / 4] = 2 << 30;
            ...
            try t.expect(!native.slots[old_native].live and run.native_active == null and native.released == prior_released + 2 and
        ...
        try t.expect(!native.slots[index].live and run.native_active == null and native.released == prior_released + iteration + 1);
...
fn checkDeviceGraphics(target: *@import("gsp_device.zig").Device, table: *a.DriverApi, scenario: []const u8) !void {
    ...
    if (native.is("context_graphics") or graphicsFaultScenario(scenario)) {
...
fn checkGraphicsRendering(target: *@import("gsp_device.zig").Device, table: *@import("r4os").abi.DriverApi, counts: *FifoCounts, scenario: []const u8) !void {
...
fn checkNvdecChannels(target: *@import("gsp_device.zig").Device, counts: *FifoCounts, scenario: []const u8) !usize {
    ...
    // Earlier renderer cases return BO references asynchronously. Establish
    // the baseline only after those pre-existing retirements have completed.
    var stage: u32 = 0;
    for (0..200) |_| {
        if (run.native_active == null and run.context_active == null and run.fifo_active == null and !native.pendingReleases()) break;
    ...
    for (std.enums.values(NvdecPeer.Mode)) |mode| {
        ...
        try t.expect(steps < 1100);
        var moved = owner;
        try t.expectError(error.State, moved.requestClose());
        if (mode == .success) {
        ...
        try owner.requestClose(); try owner.requestClose();
        steps = 0;
        while (steps < 800) : (steps += 1) {
            _ = target.step(); try t.expect(target.phase == .ready);
            _ = try owner.step(run);
            if (run.fifo_active != null) try replyDeviceFifo(target, counts, scenario)
            else if (run.activeChannel().?.phase == .waiting) try replyNativeProduct(target);
...
fn checkNativeQueues(target: *@import("gsp_device.zig").Device, table: *a.DriverApi, counts: *FifoCounts, scenario: []const u8) anyerror!usize {
    ...
    for (0..3) |round| {
        ...
        if (round == 0) { first_channel = handle; first_context = node.engine.graphics.context; }
        ...
        if (round == 2) {
            ...
            try t.expect(node.closing and node.jobs == 1 and node.engine.graphics.phase == .ready and owner.state == .handed_off);
        }
    ...
    mapped: @import("gsp_runtime.zig").VirtualBindingHandle) !void
{
    ...
    defer target.native_graphics.phase = gr_phase;
    target.native_graphics.phase = .unavailable;
    const backend = run.copy_backend.?;
    try t.expect(backend.queue.unregister(&backend.binding, 1) == 1);
    run.copy_backend = null;
    _ = try run.native_queues.step(run, null);
    _ = try run.registerCopyBackend(run.graphics_copy_channel.?, clock + std.time.ns_per_s);
    try t.expect(try run.native_queues.step(run, null));
    try t.expect(run.native_queues.ready() and run.native_queues.source == null);
    const before_heap = heap_model.heapLive();
    const before_bytes = native.charged;
    var stage: u32 = 0;
    ...
    for (0..3) |round| {
        ...
        const ticket = work.ticket.?;
        const bytes: [*]u8 = @ptrFromInt(owner.ring.cpu.cpu_address);
        @memset(&peer.command, 0xcc);
        try run.validatePushBatch();
        std.mem.writeInt(u32, bytes[layout.userd_offset + 0x88..][0..4], ticket.put, .little);
        if (round == 2) { peer.closed[index] = true; peer.notify(); }
        for (0..8) |_| _ = try stepNativeQueues(target, counts, scenario, &stage);
        try t.expect(peer.completed == completed and node.jobs == 1 and owner.state == .handed_off and
            (try run.virtuals.findBinding(mapped)).executions == 1 and run.batch_work.?.receipt == null);
        if (round == 2) try t.expect(node.closing and node.engine.video.phase == .ready);
        peer.signaled = true;
        std.mem.writeInt(u32, bytes[layout.completion_offset..][0..4], ticket.point, .little);
        for (0..80) |_| { _ = try stepNativeQueues(target, counts, scenario, &stage); if (!run.hasQueuedWork()) break; }
    ...
    for ([_]NvdecPeer.Mode{ .missing_class, .rejected, .absent }, 10..) |mode, index| {
        ...
        try t.expect(peer.completed == completed + 1 and peer.result == a.gfx_queue_result_failed and
    ...
    ce: @import("gsp_runtime.zig").ChannelHandle, image_index: usize, output: []u8) !void
{
    ...
    try t.expect(bounded.reservedBytes() == 0 and bounded.close(true));
}
fn checkRenderUpload(target: *@import("gsp_device.zig").Device, ce: @import("gsp_runtime.zig").ChannelHandle, output: []u8) !void {
    const run = &target.running;
    const fifo = @import("gsp_fifo_test_model.zig").Model;
    const ce_owner = run.fifos[ce.slot].owner.?;
    ...
    try t.expect(source_address == 0x600000 and destination == work.target_stamp.?.address and bytes == work.bytes and bytes == output.len);
    std.mem.writeInt(u32,commands[0x2088..][0..4],ticket.put,.little);
...
fn checkQueuedRendering(target: *@import("gsp_device.zig").Device, table: *@import("r4os").abi.DriverApi, counts: *FifoCounts,
    scenario: []const u8, ce: @import("gsp_runtime.zig").ChannelHandle) !void {
    // Existing owner case: solid baseline, cropped bilinear/scissor/alpha,
    // then the separate encode-sRGB shader, through the real queue runtime.
    const run = &target.running;
    const buffers = run.presentation_buffers; defer run.presentation_buffers = buffers;
    run.presentation_buffers = 3; // Configured frame count, no attached output.
    if (graphicsFaultScenario(scenario)) return checkQueuedScene(target,table,counts,scenario,ce,11);
...
fn checkQueuedScene(target: *@import("gsp_device.zig").Device, table: *@import("r4os").abi.DriverApi, counts: *FifoCounts,
    ...
    if (!scene.solid) {
        ...
        source_image = try @import("gsp_render_job.zig").image(.{ .info = info, .driver_owner = native.slots[source_index.?].reservation.driver_owner },false);
    }
    const old_memory = table.gfx_memory_query; const old_queue = table.gfx_queue_query;
    defer { table.gfx_memory_query = old_memory; table.gfx_queue_query = old_queue; }
    ...
    if (scene.solid and !graphicsFaultScenario(scenario)) {
        const gr_put = run.fifos[target.native_graphics.channel.?.slot].owner.?.ring.put;
        const ce_put = run.fifos[ce.slot].owner.?.ring.put;
        const held = model.heldReferences();
        model.enqueueRender(native_index,null,.{ .kind = 0, .filter = 2 },deadline);
        try t.expect(try run.beginCopyWork(ce,model.binding,deadline));
        for (0..32) |_| { try stepQueuedRendering(target); if (run.queued_render == null) break; }
        const rejected = &run.faults.records[(run.faults.serial-1)%16];
        try t.expect(target.phase == .ready and run.queued_render == null and model.result == a.gfx_queue_result_failed and
            !model.active and !model.lost and model.heldReferences() == held and run.graphics_completed == before and
            run.graphics_upload == null and run.graphics_work == null and run.fifos[ce.slot].owner.?.ring.put == ce_put and
            run.fifos[target.native_graphics.channel.?.slot].owner.?.ring.put == gr_put);
    ...
    for (0..rounds) |round| {
        ...
        if (round == 0 or combined) {
            ...
            for (0..20) |_| {
                try stepQueuedRendering(target);
                try t.expect(run.graphics_upload == null);
                if (run.graphics_work != null and run.graphics_work.?.submitted) break;
            }
            try t.expect(run.fifos[ce.slot].owner.?.ring.put == ce_put and run.graphics_cache.uploaded_bytes == uploaded and
                run.graphics_cache.packet_reuses == reused+1 and run.graphics_cache.program_uploads == 1);
        }
        // No second execution lease is acquired for queue-owned images.
        try t.expect(native.slots[native_index].reference != last and native.slots[native_index].gpu.lease.id == 0 and native.slots[native_index].imported);
        if (source_index) |index| try t.expect(!native.slots[index].reference and native.slots[index].gpu.lease.id == 0 and native.slots[index].imported);
        for (0..20) |_| {
            try stepQueuedRendering(target);
            if (target.phase != .ready) return error.RenderQueue;
            if (run.graphics_work != null and run.graphics_work.?.submitted) break;
        ...
        const gp = @as(usize,(ticket.put+511)%512)*8;
        const count: usize = load(gr_data,gp+4)>>10;
        const command_address = @as(u64,load(gr_data,gp))|(@as(u64,load(gr_data,gp+4)&255)<<32);
        ...
        try t.expect(target.phase == .ready and run.queued_render == null and run.graphics_work == null and
            ...
            run.graphics_completed == before+round+1 and !run.graphics_cache.borrowed);
        if (source_index) |index| try t.expect(!native.slots[index].imported and native.slots[index].gpu.lease.id == 0);
    }
    // A stale generation cannot bind a warm packet. An interrupted changed
    // packet invalidates the preceding descriptor even before CE execution.
    const old_draw = run.graphics_cache.draw.?;
...
fn checkGraphicsFault(target: *@import("gsp_device.zig").Device, ce: @import("gsp_runtime.zig").ChannelHandle,
    ...
    const native = @import("gsp_vram_test_model.zig").Model;
    const fifo = @import("gsp_fifo_test_model.zig").Model;
    const run = &target.running; const session = &target.session.?;
    const gr = run.graphics_work.?.channel_handle;
    const owner = run.fifos[gr.slot].owner.?;
    const ticket = run.graphics_work.?.ticket.?;
    const held = model.heldReferences();
    const completed = run.graphics_completed;
    const point = owner.ring.completed;
    ...
    outputWord(&payload,4,owner.parent.?.selected.?.data[3]);
    outputWord(&payload,8,0xabc);
    @memcpy(payload[12..][0..message.len],message);
    try t.expect(held > 0 and model.active and run.graphics_cache.borrowed and !native.slots[image_index].reference);
    ...
    std.mem.writeInt(u32,fifo.slots[0].data[0x2200..][0..4],ticket.point,.little);
    try t.expect(!run.graphics_cache.close(true));
    try t.expectError(error.State,run.beginCopyWork(ce,model.binding,deadline));
    try t.expectError(error.State,run.retireExecutionChannel(gr,deadline,true));
    try t.expectError(error.State,run.beginDestroyGraph(deadline,true));
    ...
    window_handle: @import("gsp_runtime.zig").DisplayChannelHandle, dma: u32, scenario: []const u8) !void
{
    ...
    for (0..2) |frame| {
        ...
        // The authenticated event now has a quiet interval. Exercise the
        // same interleaved receiver query once that real work becomes due.
        if (frame != 0) {
            try t.expect(running.receiver_events.pending);
            clock = @max(clock, running.receiver_events.dueNs());
        }
        ...
        if ((frame == 0 and (!admission_checked or target.session.?.tx_sequence != prior_tx)) or
            (frame != 0 and (output_replies != 1 or target.session.?.tx_sequence != prior_tx + output_replies)))
            std.debug.print("present admission frame={d} checked={} replies={d} tx={d}/{d} outputs={s} pending={} due={d} clock={d}\n",
                .{ frame, admission_checked, output_replies, target.session.?.tx_sequence, prior_tx,
                    @tagName(running.outputs.state), running.receiver_events.pending, running.receiver_events.dueNs(), clock });
...
fn checkDeviceOutputs(target: *@import("gsp_device.zig").Device, words: []u32, frts: u64, scenario: anytype) !void {
    ...
    while (target.phase == .ready and steps < 1200) : (steps += 1) {
        ...
        if (owner.state == .returned) {
            ...
            clock = running.receiver_events.dueNs();
            passes += 1;
            _ = target.step();
            try t.expect(owner.state == .topology and !running.receiver_events.pending);
        }
