...
pub const Receiver = struct {
    ...
    pub fn capture(report: anytype) Receiver {
        return .{ .valid = report.complete(), .digital = report.digital, .colors = report.colors,
            ...
            .hdr_eotf = report.hdr_eotf, .hdr_static = report.hdr_static, .hdmi_links = report.hdmi_links };
    }
};
...
pub fn clockHz(saved: anytype) u64 {
    return links.Clock.nvidia(saved.signal.clock).ceilHz() catch 0;
}
...
pub fn admit(saved: anytype, receiver: anytype, link: color.Link) !?color.Plan {
    ...
    const transport: color.Transport = switch (link) { .hdmi, .hdmi_frl => .hdmi, .dvi => .dvi, .displayport => .displayport };
