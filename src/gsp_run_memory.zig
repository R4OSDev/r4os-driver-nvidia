...
const map_count = gsp_dma.max_mappings + 1 + init.max_mappings + 1;
...
pub const Lease = struct {
    ...
    allocations: [4]u64 = @splat(0),
    mappings: [map_count]Stamp = undefined,
    mapped_count: usize = 0,
    retained: bool = false,
    failed: bool = false,

    fn allocationsFor(b: *boot.Storage, i: *init.Storage, f: *security.Storage) [4]a.DriverHeapAllocation {
        return .{ b.image.allocation, b.allocation, i.allocation, f.allocation };
    }
    fn mapsFor(b: *boot.Storage, i: *init.Storage, f: *security.Storage, out: *[map_count]*const a.DmaMapping) usize {
        var count: usize = 0;
    ...
    fn available(b: *boot.Storage, i: *init.Storage, f: *security.Storage, api: *const a.DriverApi) bool {
    ...
    /// All three source owners must already have completed their own firmware,
    ...
    pub fn acquire(self: *Lease, ctx: *const r4os.r4dev.DriverContext, b: *boot.Storage, i: *init.Storage, f: *security.Storage) !void {
        if (self.self_address != 0) return error.Busy;
        if (!available(b, i, f, ctx.api)) return error.Storage;
        const allocations = allocationsFor(b, i, f);
        ...
        const count = mapsFor(b, i, f, &maps);
        ...
        self.* = .{ .self_address = @intFromPtr(self), .api = ctx.api, .boot_storage = b, .init_storage = i, .fwsec_storage = f, .queue = queue, .mapped_count = count };
    ...
    fn matches(self: *const Lease) bool {
        ...
        for (allocationsFor(b, i, f), self.allocations) |allocation, expected| if (allocation.handle != expected) return false;
        var maps: [map_count]*const a.DmaMapping = undefined;
        if (mapsFor(b, i, f, &maps) != self.mapped_count) return false;
    ...
    pub fn inputs(self: *const Lease) !Inputs {
        ...
        return .{ .boot = b, .init = i, .fwsec = self.fwsec_storage.?.device.prepared_plan.?, .resume_args = .{ .libos_dma = i.init.libos_address, .app_version = b.app_version } };
    }
