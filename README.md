...
work. CPU-copy fences retain their meaning.
The optional private C67A PIO cursor channel now owns signed positions,
bounded queue pressure and physical retirement, without a DMA command buffer.
Drained PIO methods and a fresh real head observation form a separate receipt;
partial writes are never replayed. Cursor image/alpha/hotspot, common transport
and Desktop software fallback integration remain open.
...
publication, prepare_held, initial CE upload and HDMI boot-mode commit.
The shadow and common commit use the same immutable pre-firmware RAM image.
...
feasibility. Generation and full reply/ACK identity bind the result.
Rejected queries finish RM cleanup before product failure; uncertain frees
...
to the saved mode and complete current receiver facts.
...
separate common-BO command buffer and held native instance/USERD storage.
Allocate/bind/token/enable are acknowledged in order. Confirmed channel
free precedes command memory and private-use release; group methods survive
all siblings. Unknown effects retain reachable resources. Two-channel and
rejection/retirement cases pass in the51 existing groups, alongside seven
original-C packet pairs and the module build.0.79.11 T1-T4 are software
...
Ampere classes, then creates a group and synchronous context share in its
existing VA space. Acknowledged results and bounded child references gate
publication and reverse teardown; held contexts retain their parents while
unrelated contexts can retire. Closing cleanup uses cached heap ownership.
Original C vectors,51 existing groups and the module build pass. Instance/
FIFO backing, bind/schedule and actual engine consumers remain open.
Evidence: GrafikSpeicher07911.json / context_checkpoint.
...
(Kernel 0.1.154). One 12-KB system BO supplies CPU zeroing and direct DMA pages;
confirmed RM map plus ACK precedes its separate GPU residency lease. No
private heap copy or legacy DMA pin is used. The PCI adapter and GSP epoch
bind both leases. RM unmap/object frees precede GPU/DMA residency release,
CPU-map remnants, BO reference and collection. Failed admission unwinds the
confirmed prefix; uncertain effects or failed cleanup retain ownership.
All 51 existing NVIDIA groups and the build pass. The common kernel path has
52/52 display-step checks and actual SMP4 Work/closed-owner acceptance.
No execution channel uses this buffer yet; general app GPU resources, VRAM
placement and engines remain software work. The pinned 570.144 wire codec
and 736-byte original-C fixture are unchanged. Evidence:
Docs/Drivers/GrafikSpeicher07911.json / bo_work_checkpoint.
